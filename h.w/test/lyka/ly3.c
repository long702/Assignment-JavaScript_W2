...
char assignGrade(char lastInitial);
...
// Function to assign grade based on last name
char assignGrade(char lastInitial) {
    if (lastInitial == 'A' || lastInitial == 'a') return 'A';
    if (lastInitial == 'B' || lastInitial == 'b') return 'B';
    if (lastInitial == 'C' || lastInitial == 'c') return 'C';
    return '-'; // unknown
}
...
void addStudent() {
    ...
    s.grade = assignGrade(s.lastName[0]);

    students[studentCount++] = s;

    printf("Student added successfully. Assigned grade: %c\n", s.grade);
}
...
void registerStudentToCourse() {
    ...
    printf("Enter course name to register to: ");
    ...
    printf("Student registered to course successfully.\n");
}
...
void viewAllStudents() {
    ...
    for (int i = 0; i < studentCount; i++) {
        ...
        printf("ID: %d | Name: %s %s | Age: %d | Gender: %s | Address: %s | Course: %s | Grade: %c\n",
            s.id, s.firstName, s.lastName, s.age, s.gender, s.address, s.course, s.grade);
    }
}
